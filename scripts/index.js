...
export function openPopup(element) {
    ...
    document.addEventListener('keydown', closePopupEsc);//добавляем странице слушателя событий (нажатие клавишы, событие) - стрелочная функция - функция closePopupEsc с параметрами элемент и нажатая клавиша
}
...
function closePopupEsc(evt) { // передаем параметрами элемент (попап) и событие
    if (evt.key === 'Escape') {
        const openedPopup = document.querySelector('.popup_opened');
        closePopup(openedPopup);// вызвать функцию закрытия попапа с параметром элемент (попап)
    }
}
